...
// Stream event types for generateStrategyStream
type StreamEvent =
  | { type: 'perspective'; data: PerspectiveOutput }
  | { type: 'aggregation'; data: { status: string } | { text: string; tokensUsed: number } }
  | { type: 'validation'; data: { validated: boolean; issues: string[] } }
  | { type: 'complete'; data: { totalCost: number; totalLatency: number } }
